...
<body>
    <div class="container" >
        <h2>Créer un Bien en Location</h2>
        <form action="{{ route('bienlocation.storeAdmin') }}" method="POST" enctype="multipart/form-data" >
            @csrf
            ...
            <div class="form-group">
                ...
                <input type="number" id="loyer" name="loyer" value="{{ old('loyer') }}" required>
                @error('Loyer')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
